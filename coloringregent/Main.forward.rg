...
do
  for e in r_params do
	...
	-- Sod Shock
		...
		r_params[e].Vmin[0] = -9.0
		r_params[e].Vmin[1] = 0
		r_params[e].Vmin[2] = 0
                
		r_params[e].Vmax[0] = 11.0
		r_params[e].Vmax[1] = 0
		r_params[e].Vmax[2] = 0
...
do
  ...
  for s in s3 do
    ...
    for v in v3 do
      ...
      --if s.x == r_gridbarp.bounds.lo.x + 1 then rhotest += r_gridbarp[e6].g*vxmesh[v.x].w end
      --if v.x == 128 then c.printf("phi to phibarplus[%d]: {%f to %f}, {%f to %f}\n", s.x, r_grid[e6].g, r_gridbarp[e6].g, r_grid[e6].b, r_gridbarp[e6].b) end
...
do
  ...
  for s in s3 do
    ...
    var e3 : int8d = {s.x, s.y, s.z, 0, 0, 0, 0, 0}
    var sC : double = r_mesh[e3].dx -- change when copy
...
do
  ...
  for s in s3 do
    ...
    var e3 : int8d = {s.x, s.y, s.z, 0, 0, 0, 0, 0}
    var sC : double = r_mesh[e3].dy -- change when copy
...
do
  ...
  for s in s3 do
    ...
    var e3 : int8d = {s.x, s.y, s.z, 0, 0, 0, 0, 0}
    var sC : double = r_mesh[e3].dz -- change when copy
...
do
  ...
  c.sprintf([&int8](filename), './Data2/rho_%04d',iter)
